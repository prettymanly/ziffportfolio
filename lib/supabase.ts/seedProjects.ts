...
/** Seed Household OS (project_number 1) slides */
async function seedHouseholdOS() {
  const entries = [
    {
  ...
  const { error } = await supabase.from('projects').insert(entries)
  if (error) {
    console.error('Failed seeding Household OS slides:', error.message)
  } else {
    console.log('Household OS slides seeded.')
  }
}
